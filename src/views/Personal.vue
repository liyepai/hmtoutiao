    ...
    <router-link :to="`/editPersonal/${gerenxinxi.id}`">
      <div class="profile">
        ...
        <div class="profile-center">
          ...
          <div class="time">{{gerenxinxi.create_date | dateFormat}}</div>
        </div>
...
export default {
  ...
  async mounted() {
    ...
    if (res.data.message === "获取成功") {
      ...
      //修改图片  需要拼接我们服务器的基地址
      this.gerenxinxi.head_img =
        "http://127.0.0.1:3000" + this.gerenxinxi.head_img;
  ...
  methods: {
    dianji() {
      localStorage.removeItem("token"), this.$router.push({ name: "Index" });
    }
  }
};
</script>
